...
function binarysearch(list, victim; low=1, high=length(list), rev=false, by="", lt=(<), multiple_matches=false)
    ...
    while high >= low
        ...
        if list[mid] == victim
            ...
            right = rev ? searchsortedlast(list[mid:high], victim) + mid : searchsortedfirst(list[mid:high], victim) + mid
            if multiple_matches
                return left:right
            else
                return left:left
...
binarysearch([1, 3, 4, 6, 6, 9, 11], 6, multiple_matches=true)
binarysearch([1, 1, 4, 6, 8, 9, 11], 1, multiple_matches=true)

binarysearch([11, 9, 8, 6, 4, 3, 1], 1, lt=(>))
